...
for index = 1:100
    ...
    % fprintf('%.0f: ',index);
    [tau(index),SSE(index)] = Project_Algorithm(y(:,index),time); % Calls the algorithm by passing the y values for a certain thermocouple and the time vector, recieves and stores tau and SSE in seperate vectors for every thermocouple
end
...
[SSE,SST,R_squared] = Project_Regression(tau,mean_tau_values); % does the regression analysis
